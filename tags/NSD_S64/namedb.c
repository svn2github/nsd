...
namedb_create(void) {
	...
	region = region_create_custom(xalloc,
				      free, 
				      DEFAULT_CHUNK_SIZE,
				      DEFAULT_LARGE_OBJECT_SIZE, 
...
#ifdef   NSEC3
	nsec3_region = region_create_custom(xalloc,
					    free, 
					    DEFAULT_CHUNK_SIZE,
					    DEFAULT_LARGE_OBJECT_SIZE, 
					    DEFAULT_INITIAL_CLEANUP_SIZE,
	...
	nsec3_mod_region = region_create_custom(xalloc,
						free, 
						DEFAULT_CHUNK_SIZE,
						DEFAULT_LARGE_OBJECT_SIZE, 
						DEFAULT_INITIAL_CLEANUP_SIZE,
	...
	db = (namedb_type *) region_alloc_zero(region,
					       sizeof(*db));
	if (db == NULL) {
		region_destroy(region);
#ifdef   NSEC3
		region_destroy(nsec3_region);
		region_destroy(nsec3_mod_region);
#endif   /* NSEC3 */
		return NULL;
	}
...
int
zone_nsec3_domains_create(struct namedb *db,
			  struct zone *zone)
{
	if ((db == NULL) ||
	    (zone == NULL)) {
...
int
zone_nsec3_domains_destroy(struct namedb *db,
			   struct zone *zone)
{
	rbnode_t *node;
	...
	while (node != RBTREE_NULL) {
		...
		region_recycle(db->nsec3_region,
			       nsec3_domain,
			       sizeof(*nsec3_domain));
	}
...
int
namedb_add_nsec3_domain(struct namedb *db,
			...
			struct zone *zone)
{
	...
	nsec3_domain = (struct nsec3_domain *)
		region_alloc(db->nsec3_region,
			     sizeof(*nsec3_domain));
	...
	if (rbtree_insert(zone->nsec3_domains,
			  (rbnode_t *) nsec3_domain) == NULL) {
		region_recycle(db->nsec3_region,
			       nsec3_domain,
			       sizeof(*nsec3_domain));
	}
...
int
namedb_del_nsec3_domain(struct namedb *db,
			struct domain *domain,
			struct zone *zone)
{
	rbnode_t *node;
	...
	node = rbtree_delete(zone->nsec3_domains,
			     domain_dname(domain));
	...
	if (nsec3_domain->covers != NULL) {
		 ...
		 * Make sure it getsadded to the NSEC3 list later by
...
namedb_nsec3_mod_domains_create(struct namedb *db)
{
	...
	db->nsec3_mod_domains = rbtree_create(db->nsec3_mod_region,
					      dname_compare);
			    ...
			    struct domain *domain)
{
	...
	if (rbtree_insert(db->nsec3_mod_domains,
			  (rbnode_t *) nsec3_mod_domain) == NULL) {
